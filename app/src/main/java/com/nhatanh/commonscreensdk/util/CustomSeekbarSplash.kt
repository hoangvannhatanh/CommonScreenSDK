...
class CustomSeekbarSplash : View {
    ...
    private var colorPr = intArrayOf(
        Color.parseColor("#22BC5E"),
        Color.parseColor("#22BC5E"),
    )
    var onProgress: ICallBackProgress? = null

    init {
        w = resources.displayMetrics.widthPixels / 100f + 3
        sizeBg = 3.33f * w
        sizePos = 2.22f * w
        radius = 3.33f * w
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (!isFirstShader) {
            paintProgress.shader =
                LinearGradient(0f, 0f, 0f, height.toFloat(), colorPr, null, Shader.TileMode.CLAMP)
            isFirstShader = true
        }
