package com.nhatanh.commonscreensdk.extensions
...
fun isSingleCLick1000(): Boolean {
    return isClickEvent(1000)
}
...
fun View.startShineEffect() {
    val shine = GradientDrawable(
        GradientDrawable.Orientation.BOTTOM_TOP,
        intArrayOf(Color.TRANSPARENT, Color.WHITE, Color.TRANSPARENT)
    )
    shine.alpha = 0
    this.overlay.add(shine)

    val animator = ValueAnimator.ofFloat(0f, 1f)
    animator.duration = 1000
    animator.addUpdateListener { anim ->
        val progress = anim.animatedValue as Float
        val top = (1 - progress) * this.height
        val bottom = top + this.height * 0.5f
        shine.setBounds(0, top.toInt(), this.width, bottom.toInt())
        shine.alpha = (255 * (1 - abs(progress - 0.5f) * 2)).toInt()
        this.invalidate()
    }

    animator.addListener(object : AnimatorListenerAdapter() {
        override fun onAnimationEnd(animation: Animator) {
            this@startShineEffect.overlay.clear()
        }
    })
    animator.start()
}

fun triggerRandomShineEffect(recyclerView: RecyclerView) {
    ...
    val randomPosition = Random.nextInt(firstVisiblePosition, lastVisiblePosition + 1)

    val viewHolder = recyclerView.findViewHolderForAdapterPosition(randomPosition)
    viewHolder?.let {
        it.itemView.startShineEffect()
    }
}
